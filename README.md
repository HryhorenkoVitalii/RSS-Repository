...
| **Каталог `/data`** | В Compose — **именованный том** `app_media` (медиа не выносится в каталог репозитория на хосте). База — **MariaDB**, том `mariadb_data`. |
...
docker compose down -v       # + удалить тома MariaDB и app_media (чистая БД и медиа)
```
