...
Приложение подключается к SQLite с **WAL** и **busy_timeout 15&nbsp;с** — это снижает предупреждения «database is locked» при параллельной работе HTTP и фонового опроса фидов. Если база на **NFS/SMB** или общей сетевой папке, SQLite может вести себя нестабильно — лучше локальный диск.
